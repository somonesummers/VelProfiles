function [s, m2] = fitSlope(x, z, t, sMax, v, dr)
    ...
    for i = size(x,2):-1:1
        ...
        if i < size(x,2)
            bd = m(1) * v(i) / v(i + 1); %bound search to look for lower velocities
        else
            bd = m(1); 
        end
        %limit range of these bounds manually
        if(abs(bd * buff * dr / v(i)) > .2)
            bd = .2 * v(i) / (buff * dr);
        elseif(abs(bd * buff * dr / v(i)) < 1e-3)
            bd = 1e-3 * v(i) / (buff * dr);
        end
        ...
        m = m_s(:,I);
        m2(:,i) = m;
        s(i) = abs(m(1) * dr ./ v(i)); %calc output speed profile
    end
end