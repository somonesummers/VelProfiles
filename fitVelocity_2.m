function [v, m2] = fitVelocity_2(x, z, t, s, vMax, dr, m_p)
    % [v, m2] = fitVelocity(x, z, t, s, vMax, sMax, dr) solves for velocity profile
    % of a region of known layer slopes (s(z)) with prior sv fit. Velocity is assumed to be
    ...
    for i = size(x,2):-1:1
        ...
        [  m_0, fval_0  ] = fminsearchbnd(fcn, [m_p(i);      0],[-abs(bd)*buff -2*pi],[abs(bd)*buff 2*pi]);
        [ m_pi, fval_pi ] = fminsearchbnd(fcn, [m_p(i);   pi/2],[-abs(bd)*buff -2*pi],[abs(bd)*buff 2*pi]);
        [m_npi, fval_npi] = fminsearchbnd(fcn, [m_p(i);  -pi/2],[-abs(bd)*buff -2*pi],[abs(bd)*buff 2*pi]);
